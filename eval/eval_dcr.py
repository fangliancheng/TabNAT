...
if __name__ == '__main__':
    ...
    if not args.path:
        syn_path = f'{script_path}/../synthetic/{dataname}/{model}.csv'
    else:
        syn_path = args.path

    real_path = f'{script_path}/../synthetic/{dataname}/real.csv'
    test_path = f'{script_path}/../synthetic/{dataname}/test.csv'
    ...
    dcr_scores = []
    idx = np.arange(real_data.shape[0])
    seed = 1234
    for trail_idx in range(10):
        np.random.seed(seed + trail_idx)
        np.random.shuffle(idx)
        
        train_idx = idx[:test_data.shape[0]]
        sampled_real_data = real_data.iloc[train_idx]

        assert sampled_real_data.shape[0] == test_data.shape[0], 'The number of real data and test data should be the same'
        ...
        sampled_real_data.columns = list(np.arange(len(sampled_real_data.columns)))
        syn_data.columns = list(np.arange(len(sampled_real_data.columns)))
        test_data.columns = list(np.arange(len(sampled_real_data.columns)))
        for i in num_col_idx:
            num_ranges.append(sampled_real_data[i].max() - sampled_real_data[i].min()) 
        
        num_ranges = np.array(num_ranges)

        num_real_data = sampled_real_data[num_col_idx]
        cat_real_data = sampled_real_data[cat_col_idx]
        ...
        sampled_real_data_th = torch.tensor(real_data_np).to(device)
        ...
        for i in range((syn_data_th.shape[0] // batch_size) + 1):
            ...
            dcr_real = (batch_syn_data_th[:, None] - sampled_real_data_th).abs().sum(dim = 2).min(dim = 1).values
        ...
        print(f'DCR Score trail {trail_idx}, a value closer to 0.5 is better')
        print(f'{dataname}-{model}, DCR Score = {score}')

        dcr_scores.append(score)

        # ratio = num_real_data_np.shape[0] / (num_real_data_np.shape[0] + num_test_data_np.shape[0])
        # print(f'{dataname}-{model}, Normalized DCR Score = {score/ratio}')

    print(f'{dataname}-{model}, Average DCR Score = {np.mean(dcr_scores)}')