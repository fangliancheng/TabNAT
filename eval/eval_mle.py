...
if __name__ == '__main__':
    ...
    if task_type == 'regression':
        ...
        best_f1_scores, best_weighted_scores, best_auroc_scores, best_acc_scores, best_avg_scores = evaluator(train,
                                                                                                              test,
                                                                                                              info)

        overall_scores = {}
        for score_name in ['best_f1_scores', 'best_weighted_scores', 'best_auroc_scores', 'best_acc_scores',
                           'best_avg_scores']:
            overall_scores[score_name] = {}
    ...
    if info['task_type'] == 'regression':
        rmse = overall_scores['best_rmse_scores']['XGBRegressor']['RMSE']
        print(f'RMSE: {rmse:.3f}')
    else:
        auc = overall_scores['best_auroc_scores']['XGBClassifier']['roc_auc']
        print(f'AUC: {auc:.3f}')
