...
def main(args):
    ...
    num_cat = len(info['cat_col_idx']) 
    is_num_only = True if num_cat == 0 else False

    task_type = info['task_type']
    cat_encoding = args.cat_encoding
    concat = True if task_type == 'regression' else False

    #concat = True # Liancheng: set the same as TabMAR
    ...
    if task_type == 'binclass':
        ...
        sm = SMOTE(k_neighbors=min(5, len(X_train_0)-1))
        X_res, y_res = sm.fit_resample(X_src, y_src)
        print('Original dataset shape %s' % Counter(y_src))
        print('Resampled dataset shape %s' % Counter(y_res))
        syn_X0 = X_res[-X_train_0.shape[0]:]
        # Liancheng 
        syn_y0 = y_res[-X_train_0.shape[0]:]
        syn_X0 = np.concatenate((syn_X0, syn_y0.reshape(-1, 1)), axis = 1)
        '''
        ...
        sm = SMOTE(k_neighbors=min(5, len(X_train_1)-1))
        X_res, y_res = sm.fit_resample(X_src, y_src)
        print('Original dataset shape %s' % Counter(y_src))
        print('Resampled dataset shape %s' % Counter(y_res))
        syn_X1 = X_res[-X_train_1.shape[0]:]
        # Liancheng 
        syn_y1 = y_res[-X_train_1.shape[0]:]
        syn_X1 = np.concatenate((syn_X1, syn_y1.reshape(-1, 1)), axis = 1)

        num_num_col = len(info['num_col_idx'])

        syn_X0_num = syn_X0[:,:num_num_col]
        syn_X1_num = syn_X1[:,:num_num_col]
        syn_X0_cat = syn_X0[:,num_num_col:-1]
        syn_X1_cat = syn_X1[:,num_num_col:-1]

        if syn_X0_num is not None:
            num_inverse = dataset.num_transform.inverse_transform
            recover_X0_num = num_inverse(syn_X0_num)
            recover_X1_num = num_inverse(syn_X1_num)
            syn_num = np.concatenate((recover_X0_num, recover_X1_num), axis = 0)
    ...
    if task_type == 'multiclass':
        ...
        for current_class in unique_classes:
            class_idx = (y_train == current_class).nonzero()[0]
            print('length of class idx: ', len(class_idx))
            X_train_class = X_train[class_idx]
            syn_y.append(y_train[class_idx])

            X_new = np.random.randn(X_train_class.shape[0] * 2, X_train_class.shape[1])
            y = np.ones((X_train_class.shape[0]))
            y_new = np.zeros((X_train_class.shape[0] * 2))

            X_src = np.concatenate((X_train_class, X_new), axis=0)
            y_src = np.concatenate((y, y_new), axis=0)

            sm = SMOTE(k_neighbors=min(5, len(X_train_class)-1))
            X_res, y_res = sm.fit_resample(X_src, y_src)
            print('Original dataset shape %s' % Counter(y_src))
            print('Resampled dataset shape %s' % Counter(y_res))

            syn_X_class = X_res[-X_train_class.shape[0]:]
            syn_y_class = y_res[-X_train_class.shape[0]:]
            syn_X_class = np.concatenate((syn_X_class, syn_y_class.reshape(-1, 1)), axis=1)

            synthetic_data.append(syn_X_class)

        synthetic_data = np.concatenate(synthetic_data, axis=0)

        num_num_col = len(info['num_col_idx'])

        syn_X_num = synthetic_data[:, :num_num_col]
        syn_X_cat = synthetic_data[:, num_num_col:-1]
        syn_y = np.concatenate(syn_y, axis=0)

        if syn_X_num.size != 0:
            num_inverse = dataset.num_transform.inverse_transform
            recover_X_num = num_inverse(syn_X_num)
            syn_num = recover_X_num
        else:
            syn_num = None

        if syn_X_cat.size != 0:
            cat_inverse = dataset.cat_transform.inverse_transform
            recover_X_cat = cat_inverse(syn_X_cat)
            syn_cat = recover_X_cat
        else:
            syn_cat = None
        ...
        sm = SMOTE(k_neighbors=min(5, len(X_train)-1))
        X_res, y_res = sm.fit_resample(X_src, y_src)
        print('Original dataset shape %s' % Counter(y_src))
        print('Resampled dataset shape %s' % Counter(y_res))
